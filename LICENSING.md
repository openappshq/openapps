...
- keep the successfully read Keychain record as a recovery candidate;
- immediately run an authoritative check for it;
- `valid: true` rebuilds the journal and unlocks: write the new journal to a temporary file, atomically replace the old one, and only then keep the corrupt copy aside;
- `valid: false` records the revocation;
