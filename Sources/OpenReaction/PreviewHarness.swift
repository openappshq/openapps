...
/// permissions, for visual checks: `OpenReaction --preview-picker [--strip]`.
@MainActor
final class PreviewHarness {
    private let window: NSWindow
    private let picker = PickerPanelController()

    init(provider: any SuggestionProvider, layout: PickerLayout) {
        ...
        picker.model.layout = layout
        picker.model.onChoose = { [weak self] index in self?.picker.model.selectedIndex = index }
        let caret = CGRect(x: frame.minX + 132, y: frame.minY + 300, width: 0, height: 20)
        ...
        picker.present(provider.suggestions(for: "sm", recents: [], limit: PickerMetrics.maxRows), caret: caret)
