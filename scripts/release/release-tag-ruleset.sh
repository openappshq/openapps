#!/usr/bin/env bash
# The repository ruleset that makes an app's release tags immutable once they
# exist (no moves, no force pushes, no deletion, nobody exempt), so a release
# can never end up carrying binaries built from a different commit. The
# definition is a file under .github/rulesets/, one per app.
#
#   scripts/release/release-tag-ruleset.sh check <definition.json> [owner/repo]   # exit 1 unless an equivalent active ruleset exists
#   scripts/release/release-tag-ruleset.sh apply <definition.json> [owner/repo]   # create it (needs admin on the repository)
#
# Each app's release workflow runs `check` before publishing and refuses to
# publish without it; `apply` is a one-time admin step (RELEASES.md).
#
...
DEFINITION="${2:?path to the ruleset definition, e.g. .github/rulesets/openklack-release-tags.json}"
REPO="${3:-${GH_REPO:-$(gh repo view --json nameWithOwner --jq .nameWithOwner)}}"
test -f "$DEFINITION" || { echo "error: $DEFINITION not found" >&2; exit 1; }
...
administration (see RELEASES.md).
MSG
        return 1
    fi
    cat >&2 <<MSG
...
with: scripts/release/release-tag-ruleset.sh apply $DEFINITION $REPO  (see RELEASES.md)
MSG
    return 1
}
...
case "$ACTION" in
    ...
    apply)
        ...
        check
        ;;
    *) echo "usage: $0 check|apply <definition.json> [owner/repo]" >&2; exit 2 ;;
esac
