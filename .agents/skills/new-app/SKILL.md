...
Verify locally before the first publish: signing dry run with a throwaway certificate, `verify-release.sh`, `actionlint`, `shellcheck`, `scripts/release/tests/changes.test.sh` (add the app's rows), then a `publish=false` workflow run.
