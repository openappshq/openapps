...
| Screen saver | **Install screen saver** (Settings → Desktop) copies `macPaper.saver` from the app into `~/Library/Screen Savers/`; the saver shows the applied stills (and, with favorites, crossfades through them every minute) and needs nothing from the app while it runs. macOS's screen-saver picker is where it is chosen; macPaper cannot select it | not installed |
