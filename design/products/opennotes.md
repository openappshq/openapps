...
- **Drop to create:** text, a link or files dragged onto the pill, or onto the fan and its `+` tab, make one note and open it with the caret at the end, through the very path the hotkey takes. Text is the body as dropped; a link is one line, the address as written (no title is fetched — no network); each file is one line, `[name](file:///…)`, the file left where it is (never read, never copied); several items go on their own lines, in order. The pill lights up as the target while something that would make a note hovers over it (the `+` tab with the fan out); an image or an empty string is not a target. Read-only refuses: the pill does not light, the license line shows under the deck with its lock while the drag hovers, and nothing is made — the license is asked as the drag arrives and again at the drop. The open note's text takes its own drops as text, as any text view does.
- **Checklist progress:** a note whose body has `- [ ]` / `- [x]` boxes shows the count on its tab (`3/7`, at the tab's foot) and a thin line of the note's ink along the tab's inner edge, filled as far as the list has come — from the parsed body, the same parse the styler and the click use: nested items count, `[X]` is done, a `[ ]` that is no box (inside a code span, after no list marker) is not counted. Nothing is written to the file, and done items stay where the user put them (the file's order is the user's). Ticking the last box ticks the count once (a spring, out and back; Reduce Motion shows the new count with no movement). A note whose whole body is not in memory (evicted, or cut at the read cap) shows no count rather than a wrong one.
