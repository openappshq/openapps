...
| Fan | Tabs shingled down the edge, 40 pt wide, one per active note (8 at most, then "+N"), the tab in the note's paper for the appearance with the title reading down it in the paper's ink; a `+` tab at the end | The pill, after the pointer has rested on the edge; the hotkey while read-only; an open note closing | The pointer leaving the edge and the deck for 350 ms; a tab click; Escape |
...
Rich text, images, fonts that aren't installed on the Mac (nothing is bundled beyond the three faces), a database, our own sync, a notch surface, encryption of the files (FileVault does that), per-app notes, AutoPaste, inline math, OCR, timers, widgets, iPhone. Some are later tickets; none changes the file format.
