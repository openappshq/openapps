...
Open System Settings (in the guide and on the home screen's permission notice) also shows a floating drag-to-grant helper beside System Settings: the app icon to drag straight into the Input Monitoring list when OpenKlack is not listed, the instruction "Drag this icon into the Input Monitoring list, then turn it on", "Not working? Reset" (clears OpenKlack's own entry with `tccutil` and asks again, so macOS re-prompts and a fresh entry appears) and Close. The guide's step reads "In System Settings, turn on OpenKlack in the list. Not in the list? Drag the icon from the floating window into it." and offers "Show the helper again" while the helper is away. The helper never takes focus from System Settings, closes itself once the permission is granted, and goes away when the step is left (Back, Continue, Skip, Done) or the app quits.
