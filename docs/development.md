...
The service follows the contract's write order: losing access takes effect in memory first and is then saved (a failed save is retried every tick and shown as a storage error), while gaining access is saved first and takes effect only once that save has landed.
A revocation is also noted in a small non-secret journal outside the Keychain (`license-journal.json` in the app's data directory), keyed by a SHA-256 hash of the activation ID and holding only the revocation time, written before the Keychain save; on load, an entry for the stored activation forces Revoked whatever the record says (unless Dodo granted that activation after the entry was written, in which case the stale entry is dropped). Removing or replacing an activation writes the same note as a tombstone, and an entry is cleared only once the revoked, cleared or replaced record has been saved, or on `valid: true` for that activation; a journal write that fails keeps access off in memory, shows a storage error and is retried every tick. It never contains the license key.
