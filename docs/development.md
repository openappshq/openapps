...
The certificate and the tokens are **repository** secrets shared by every app ([RELEASES.md, Signing material](../RELEASES.md#signing-material)), read by name, never copied per app:
...
Then create the GitHub environment `openklack-release` (deployment branches and tags restricted to `main` and `openklack-v*`) with only the app's own secrets:
