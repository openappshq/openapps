...
The manager runs on its own global actor (`LicenseActor`), never on the main actor, so a Keychain or preferences call that stalls cannot stall the UI, the deadline timers or the tap's stop. It hands a `LicenseSnapshot` to the app layer (`onChange`) right after its memory changes and *before* it touches storage; `LicenseController` derives the entitlement from that snapshot with the clock (`snapshot.state(now:)`), so deadlines never wait on I/O, and a snapshot that turns the feature off pulls `AppController.featureLock()` from the manager's thread at once — `runner.beginShutdown()` under the runner's lock, which refuses any replacement still queued on the insertion queue — while the tap's stop and the UI follow on main. A tap installed later resets a shutdown begun while none ran (`tapStarted`). Until the manager has loaded, there is no record: the feature is off.
