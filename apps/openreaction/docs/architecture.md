...
`LICENSING.md` at the repository root is the contract. Source builds compile licensing out (`OPENAPPS_LICENSING` unset): no License UI, no trial, no license or registry network calls, every feature on. `scripts/bundle.sh` opts in with `OPENAPPS_LICENSING=1` and generates `LicensingConfig.swift` (Dodo host, the paid product ID, the trial registry URL) — the build fails loudly without a real product ID.
