...
The setup window opens by itself once, on the first launch, and after a relaunch it started (`OnboardingLaunch`, core, tested with an in-memory store); afterwards it is reached from Settings ("Show setup guide"), the status menu ("Setup Guide…", or "Finish Setup…" with a badge on the icon while a permission is missing) and by reopening the app. Every step can be skipped; progress (started, practice finished) is kept, so the guide resumes where it was left. Official builds also register the login item once, on the first launch of a demonstrably fresh install (`LoginItemDefault`): no preferences from an earlier launch of any version, and the licensing manager's first read found neither a trial nor a license record in the Keychain (`LicenseSnapshot.freshInstall`, positive "not found" answers only — an unreadable Keychain decides nothing yet). An upgrade, a reinstall over a kept Keychain, or a login item the user once turned off is left alone; the decision is recorded so it never runs again, and the Settings toggle records itself first, so a choice made while the Keychain is still being read is never undone either.

## 9. Licensing (official builds only)

`LICENSING.md` at the repository root is the contract. Source builds compile licensing out (`OPENAPPS_LICENSING` unset): no License UI, no trial, no license or registry network calls, every feature on. Official builds show the license where the user looks: `LicenseBadge` (core) maps the state to one short line — the trial's remaining time, "Trial ended", the short reason a restricted state keeps the picker off, nothing while licensed — which the settings window carries as a pill in its title bar, the status menu as a clickable line and the welcome step of the setup guide; each leads to Settings → License. It is derived from the published `LicenseSnapshot`, never from storage. `scripts/bundle.sh` opts in with `OPENAPPS_LICENSING=1` and generates `LicensingConfig.swift` (Dodo host, the paid product ID, the trial registry URL) — the build fails loudly without a real product ID.

The rules live in `OpenReactionCore/Licensing`: `LicensePolicy` (daily check, one-week grace with a warning after five days, clock-rollback guard, backoff 1 min → 1 h, and the trial's state from its record) and `LicenseManager` (activation with the product check that deactivates anything but the paid product, checks where only `valid: false` revokes, removal, rate limits, and the in-app trial), driven by a `LicenseClient`, `LicenseStore`, `TrialStore`, `TrialRegistryClient` and `DeviceIdentity` with an injectable clock; the 27 shared cases are `LicensingTests` and `TrialLicensingTests`, with the timing ones in `LicenseEnforcementTests`. The app supplies `DodoLicenseClient` (URLSession, public endpoints, activation name "Mac"), `KeychainLicenseStore` (service `space.openapps.openreaction.license`; activations still owed a deactivation are kept until Dodo confirms), `KeychainTrialStore` (service `space.openapps.openreaction.trial`, never deleted by the app), `URLSessionTrialRegistryClient` and `PlatformDeviceIdentity` (`IOPlatformUUID`), and `LicenseController` (launch check in the background, then one `tick` on every timer, wake, clock change and network return). A locked state stops only the picker: `AppController` does not start the tap, while the menu bar, Settings, License and Quit keep working. `openreaction://activate?key=…` only pre-fills the key; the user confirms.

The trial needs no signup. A license always wins; without a license record (read, not merely unreadable) the trial record decides. On load, a trial record that is positively absent becomes a provisional one (`started_at = last_seen_at = now`, `registered = false`), saved before the core turns on — a refused save runs nothing and is retried, reading again first — and a record that cannot be read is a storage error that never creates a trial or calls the registry. An unregistered trial posts `{app, device, env}` to the registry on launch, on ticks with backoff (1 min → 1 h, `Retry-After` honored) and at once on wake, network return or Try again; `device` is the SHA-256 of `openapps-trial-v1:openreaction:<IOPlatformUUID>`, or of a random id kept in the trial record when the UUID cannot be read (saved before it is ever sent). The answer is converted to the local clock and only an earlier start is adopted; an answer that lets access last longer (the 24 h limit lifted, or the trial back on) is saved first and published only once saved — the provisional limit keeps applying meanwhile and the save is retried on ticks without asking again — while one that shortens or ends the trial lands in memory first. Elapsed time is `last_seen_at − started_at` (or the wall clock when it is already past): the trial ends at 3 days and an unregistered one pauses at 24 h ("connect to continue"); both are deadlines derived from the snapshot, never from a save or the network. Trial time lives in one `TrialClock` value (core): an anchor `(wall, CLOCK_MONOTONIC, seen)` and a single `observe` step that sets `seen = max(seen + monotonic time since the anchor, wall)` exactly once and re-anchors. Readings are taken inside the serialized step, after any wait, and an observation older than the anchor is ignored, so the anchor only moves forward. Ticks, wake, launch and applying a registry answer observe through it and nothing else moves `last_seen_at`; a failed save never moves the anchor. The snapshot carries the clock and enforcement projects it (`state(now:uptime:)`, `deadlineDelay`), so a trial ends on monotonic time even while the manager waits on storage or the network, and the deadline timer is keyed off that projection. A wake is projected by the app layer from the snapshot at once (`wakeSince`) and observed by the manager before anything queued. `last_seen_at` is saved hourly (monotonic), once at the end, and on Quit (bounded to 2 s). If at launch or wake the clock is more than an hour behind, the core stays off ("your Mac's clock is behind") and every trial write is frozen — no saves, no registry request, no answer applied — until the manager's own observation finds it within the hour again (a held or cached snapshot marked behind may restrict but never unlocks, so if the manager is blocked on storage the Mac stays locked until it returns); an answer that arrives meanwhile is kept as the registry sent it and converted later from the observed time (`seen − registry age − monotonic time since it arrived`). Registry backoff and `Retry-After` also run on the monotonic clock. A stored license record for a retired trial key or any non-paid product is not a license. A registered trial and any licensed Mac never call the registry. Removing a license returns to Trial or TrialEnded from the trial record. Debug builds can shorten a trial "day" with `OPENREACTION_DEBUG_TRIAL_DAY_SECONDS`.

The manager runs on its own global actor (`LicenseActor`), never on the main actor, so a Keychain or preferences call that stalls cannot stall the UI, the deadline timers or the tap's stop. It hands a `LicenseSnapshot` to the app layer (`onChange`) right after its memory changes and *before* it touches storage — the snapshot is built from memory only (the trial record is kept in memory once read), and a revocation is published, then journaled, then saved; `LicenseController` derives the entitlement from that snapshot with the clock (`snapshot.state(now:)`), so deadlines never wait on I/O, and a snapshot that turns the feature off pulls `AppController.featureLock()` from the manager's thread at once — a `FeatureLock` that resolves the runner when pulled (it is handed out before `start()` creates the runner) and calls `runner.beginShutdown()` under the runner's lock, which refuses any replacement still queued on the insertion queue — while the tap's stop and the UI follow on main. A `valid: true` takes effect only once the Keychain holds the grant (a refused save leaves the Mac as it was and counts as a failed check); check results publish once their retry and rate-limit state is settled, with an absolute next-check date. A tap installed later resets a shutdown begun while none ran (`tapStarted`). Until the manager has loaded, there is no record: the feature is off.
