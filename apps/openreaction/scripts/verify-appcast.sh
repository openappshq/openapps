...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/openreaction-verify-feed.XXXXXX")"
...
[[ "$(field openapps:app)" == openreaction && "$(field openapps:channel)" == stable ]] || { echo "FAIL: not an OpenReaction stable feed" >&2; exit 1; }
