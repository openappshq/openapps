...
# https://openapps.space/updates/openreaction/appcast.xml to announce the
...
ZIP_URL="https://github.com/openappshq/openapps/releases/download/openreaction-v${VERSION}/OpenReaction-${VERSION}.zip"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/openreaction-live.XXXXXX")"
...
curl -fsSL --retry 5 --retry-delay 10 -o "$WORK/OpenReaction-${VERSION}.zip" "$ZIP_URL"
actual="$(sha256_of "$WORK/OpenReaction-${VERSION}.zip")"
...
read -r version _ url sha256 < <(scripts/verify-appcast.sh "$WORK/appcast.xml" "$WORK/OpenReaction-${VERSION}.zip")
