...
#
#   SPARKLE_ED_PRIVATE_KEY=<base64 key> scripts/make-appcast.sh dist/OpenReaction-1.2.3.zip
#
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/openreaction-appcast.XXXXXX")"
...
PLIST="$WORK/unzipped/OpenReaction.app/Contents/Info.plist"
test -f "$PLIST" || { echo "error: ${ZIP} does not contain OpenReaction.app" >&2; exit 1; }
...
DOWNLOAD_URL="${DOWNLOAD_URL:-https://github.com/openappshq/openapps/releases/download/openreaction-v${VERSION}/OpenReaction-${VERSION}.zip}"
PUBLISHED_AT="${PUBLISHED_AT:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}"
NOTES="${NOTES:-OpenReaction ${VERSION}. Release notes: https://github.com/openappshq/openapps/releases/tag/openreaction-v${VERSION}}"
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:openapps="https://openapps.space/xml-namespaces/releases">
    <channel>
        <title>OpenReaction</title>
        <link>https://openapps.space/openreaction/</link>
        <description>OpenReaction releases</description>
        <language>en</language>
        <item>
            <title>OpenReaction ${VERSION}</title>
            <openapps:app>openreaction</openapps:app>
