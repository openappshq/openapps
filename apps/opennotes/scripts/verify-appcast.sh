...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/opennotes-verify-feed.XXXXXX")"
...
[[ "$(field openapps:app)" == opennotes && "$(field openapps:channel)" == stable ]] || { echo "FAIL: not a OpenNotes stable feed" >&2; exit 1; }
