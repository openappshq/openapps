...
# publishing").
#
#   scripts/verify-release.sh dist/OpenNotes-1.2.3.zip             # local, ad-hoc OK
#   scripts/verify-release.sh --release dist/OpenNotes-1.2.3.zip   # release: everything must pass
#
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/opennotes-verify.XXXXXX")"
...
[[ "$(info LSUIElement)" == "true" ]] || { echo "error: LSUIElement is not set; OpenNotes is a menu-bar app" >&2; exit 1; }
...
# Updates come from the signed feed only (RELEASES.md); the debug preview
# harness is compiled out of a release build.
...
if has 'dodopayments.com'; then
    ...
    [[ -n "$(info CFBundleURLTypes)" ]] || { echo "error: the opennotes:// URL scheme is missing" >&2; exit 1; }
else
...
if [[ "$REQUIRE_RELEASE" == 1 || -n "$feed" ]]; then
    [[ "$feed" == "https://openapps.space/updates/opennotes/appcast.xml" ]] || { echo "error: SUFeedURL is '${feed}'" >&2; exit 1; }
    ...
    [[ -n "$(info CFBundleURLTypes)" ]] || { echo "error: the opennotes:// URL scheme is missing" >&2; exit 1; }
    if grep -Fq 'OPENNOTES_UPDATE_TEST_ACTION' "$STRINGS"; then
    ...
    if grep -Fq '/updates/opennotes/' "$STRINGS"; then
