...
#
#   SPARKLE_ED_PRIVATE_KEY=<base64 key> scripts/make-appcast.sh dist/OpenNotes-1.2.3.zip
#
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/opennotes-appcast.XXXXXX")"
...
PLIST="$WORK/unzipped/OpenNotes.app/Contents/Info.plist"
test -f "$PLIST" || { echo "error: ${ZIP} does not contain OpenNotes.app" >&2; exit 1; }
...
DOWNLOAD_URL="${DOWNLOAD_URL:-https://github.com/openappshq/openapps/releases/download/opennotes-v${VERSION}/OpenNotes-${VERSION}.zip}"
PUBLISHED_AT="${PUBLISHED_AT:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}"
NOTES="${NOTES:-OpenNotes ${VERSION}. Release notes: https://github.com/openappshq/openapps/releases/tag/opennotes-v${VERSION}}"
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:openapps="https://openapps.space/xml-namespaces/releases">
    <channel>
        ...
        <link>https://openapps.space/opennotes/</link>
        <description>OpenNotes releases</description>
        <language>en</language>
        <item>
            <title>OpenNotes ${VERSION}</title>
            <openapps:app>opennotes</openapps:app>
...
echo "==> Wrote ${OUT} (OpenNotes ${VERSION}, build ${BUILD})"
