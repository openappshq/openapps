...
#      the update-test variant (bundle id com.openappshq.opennotes.updatetest,
...
# (OpenApps/opennotes-updatetest, where the variant also keeps its notes
# folder; the variant never touches the real app's, nor ~/Documents/OpenNotes).
...
# it runs: a status item and the deck's panel at the screen edge; it writes
# nothing outside its own folder, and asks for no permission).
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [[ "${1:-}" == "--build-signed" ]]; then
    ...
    build() { # <version> <destination>
        ...
        # combination anyway).
        VERSION="$1" OPENAPPS_LICENSING=0 OPENAPPS_OFFICIAL=1 OPENNOTES_UPDATE_TEST=1 \
        ...
        rm -rf "$2"
        mkdir -p "$(dirname "$2")"
        ditto build/OpenNotes.app "$2"
    }
    build 1.0.0 "$TMP/Applications/OpenNotes.app"
    build 1.0.1 "$TMP/B/OpenNotes.app"
    exit 0
fi
...
BUNDLE_ID="com.openappshq.opennotes.updatetest"
SCRATCH_PATH="${SCRATCH_PATH:-.build/update-test}"
export SCRATCH_PATH OPENSSL

if pgrep -f "opennotes-update-e2e\..*/OpenNotes\.app/Contents/MacOS/OpenNotes" >/dev/null; then
    echo "error: an update-test app from an earlier run is still running; quit it first (pkill -f opennotes-update-e2e)" >&2
    exit 1
fi
TMP="$(mktemp -d "${TMPDIR:-/tmp}/opennotes-update-e2e.XXXXXX")"
...
cleanup() {
    ...
    pkill -f "opennotes-update-e2e\..*/OpenNotes\.app/Contents/MacOS/OpenNotes" 2>/dev/null
    ...
    rm -rf "$HOME/Library/Caches/$BUNDLE_ID" "$HOME/Library/Application Support/$BUNDLE_ID" \
        "$HOME/Library/Application Support/OpenApps/opennotes-updatetest" \
        "$HOME/Library/HTTPStorages/$BUNDLE_ID" "$TMP" build/OpenNotes.app
...
for app in "$TMP/Applications/OpenNotes.app" "$TMP/B/OpenNotes.app"; do
...
[[ "$(plutil -extract CFBundleVersion raw -o - "$TMP/Applications/OpenNotes.app/Contents/Info.plist")" == 1000000 ]]
[[ "$(plutil -extract CFBundleVersion raw -o - "$TMP/B/OpenNotes.app/Contents/Info.plist")" == 1000001 ]]
# A wrongly signed app must not satisfy the requirement (the pinned check has teeth).
cp -R "$TMP/B/OpenNotes.app" "$TMP/wrong.app"
...
ditto "$TMP/Applications/OpenNotes.app" "$TMP/A/OpenNotes.app"

echo "==> 3. Zip, update-sign and announce B"
(cd "$TMP/B" && ditto -c -k --sequesterRsrc --keepParent OpenNotes.app "$TMP/serve/OpenNotes-1.0.1.zip")
SPARKLE_ED_KEY_FILE="$TMP/update-key/sparkle-ed25519.key" UPDATE_PUBLIC_ED_KEY="$PUBLIC_KEY" \
    DOWNLOAD_URL="http://127.0.0.1:${PORT}/OpenNotes-1.0.1.zip" OUT="$TMP/serve/appcast.xml" \
    scripts/make-appcast.sh "$TMP/serve/OpenNotes-1.0.1.zip"
...
APP="$TMP/Applications/OpenNotes.app"
BIN="$APP/Contents/MacOS/OpenNotes"
...
run_app() { # <log> <action>: runs A until it quits by itself (or 2 minutes pass)
    ...
    OPENNOTES_UPDATE_TEST_ACTION="$2" "$BIN" > "$1" 2>&1 &
    APP_PID=$!
...
still_1_0_0() { # <label>
    ...
    if [[ -e "$TMP/Applications/.OpenNotes.app.update" ]]; then
...
grep -q 'opennotes-update-test: available 1.0.1' "$TMP/run-fresh.log" || { echo "error: the fresh install did not find 1.0.1:" >&2; cat "$TMP/run-fresh.log" >&2; exit 1; }
grep -q 'opennotes-update-test: cycle-finished ok' "$TMP/run-fresh.log" || { echo "error: the launch check did not finish cleanly:" >&2; cat "$TMP/run-fresh.log" >&2; exit 1; }
if grep -q 'opennotes-update-test: downloading\|opennotes-update-test: staged' "$TMP/run-fresh.log"; then
...
if requests_since_probes | grep -q '"GET /OpenNotes-1.0.1.zip'; then
...
if grep -q 'opennotes-update-test: cycle' "$TMP/run-upgrade.log"; then
...
grep -q 'opennotes-update-test: revoked-during-download' "$TMP/run-revoke-download.log" \
    || { echo "error: the download was never revoked:" >&2; cat "$TMP/run-revoke-download.log" >&2; exit 1; }
grep -q 'opennotes-update-test: staged' "$TMP/run-revoke-download.log" && { echo "error: the update was staged after the revocation" >&2; exit 1; }
...
grep -q 'opennotes-update-test: staged' "$TMP/run-revoke-staged.log" || { echo "error: the update was never staged:" >&2; cat "$TMP/run-revoke-staged.log" >&2; exit 1; }
grep -q 'opennotes-update-test: revoked-after-staged' "$TMP/run-revoke-staged.log" || { echo "error: the staged update was never revoked" >&2; exit 1; }
...
grep -q 'opennotes-update-test: ready 1.0.1' "$TMP/run-update.log" || { echo "error: A never reported the update ready:" >&2; cat "$TMP/run-update.log" >&2; exit 1; }
if ! requests_since_probes | grep -q '"GET /appcast.xml' || ! requests_since_probes | grep -q '"GET /OpenNotes-1.0.1.zip'; then
...
test ! -e "$TMP/Applications/.OpenNotes.app.update"
test ! -e "$TMP/Applications/.OpenNotes.app.previous"
codesign --verify --deep --strict "$APP"
...
rm -rf "$APP"
ditto "$TMP/A/OpenNotes.app" "$APP"
[[ "$(plutil -extract CFBundleShortVersionString raw -o - "$APP/Contents/Info.plist")" == "1.0.0" ]]
run_app "$TMP/run-restart.log" restart
grep -q 'opennotes-update-test: ready 1.0.1' "$TMP/run-restart.log" || { echo "error: no update staged before the restart:" >&2; cat "$TMP/run-restart.log" >&2; exit 1; }
