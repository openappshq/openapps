...
struct UpdateStatusTests {
    ...
    @Test func theRowNamesTheUpdateAndItsOneAction() {
        #expect(UpdateCopy.line(for: .available(version: "0.2.1")) == "OpenNotes 0.2.1 available")
    ...
    func theModelCarriesTheStatus() {
        ...
        let folder = FileManager.default.temporaryDirectory.appendingPathComponent("opennotes-updates-status-\(UUID().uuidString)", isDirectory: true)
...
struct UpdatesWiringTests {
    private func makeUpdates(flags: UpdaterMemoryFlags) throws -> (Updates, cleanup: () -> Void) {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("opennotes-updates-wiring-\(UUID().uuidString)", isDirectory: true)
        let bundle = directory.appendingPathComponent("OpenNotes.app", isDirectory: true)
        ...
        let configuration = UpdaterConfiguration(
            ...
            feedURL: URL(string: "https://openapps.space/updates/opennotes/appcast.xml")!,
    ...
    @Test func eachPhaseMapsToTheRowsHint() throws {
        let item = UpdateFeedItem(
            ...
            notes: "", url: URL(string: "https://example.invalid/OpenNotes-0.2.1.zip")!, length: 1, sha256: String(repeating: "0", count: 64), signature: ""
        )
