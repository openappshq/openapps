...
/// State → what OpenNotes shows: the deck and every note stay readable while
/// the feature is off, one card (All Notes) or its notice (the note's
/// footer) in place of writing (design/products/opennotes.md, "Licensing").
...
struct RestrictionTests {
    ...
    @Test("The trial's end says so and sells, with a place for a key, and keeps every note read-only rather than hidden")
    func trialEnded() throws {
        ...
        #expect(card.detail.contains("read-only"))
...
/// The pill's text comes from the package, with OpenNotes' name where the
...
struct BadgeTests {
    ...
    @Test func graceWarningNamesOpenNotes() {
        let label = LicenseBadge.label(for: .grace(daysLeft: 2, showWarning: true), appName: Licensing.appName)
        #expect(label?.text == "Connect to the internet within 2 days to keep using OpenNotes")
        #expect(label?.tone == .attention)
    }

    @Test("The app's values are the ones every record, hash and suite are keyed by")
    func appValues() {
        #expect(Licensing.appID == "opennotes")
        #expect(Licensing.appName == "OpenNotes")
        #expect(Licensing.journalSuite == "space.openapps.opennotes.license")
        #expect(Updating.appID == "opennotes")
        #expect(Updating.upgradeCommand == "brew upgrade --cask opennotes")
        #expect(LicensingCopy.privacy.contains("your notes, and how you use OpenNotes are never sent"))
...
struct GuideCopyTests {
    ...
    @Test func followsTheState() {
        ...
        #expect(GuideCopy.licenseLine(state: .trialEnded)?.contains("read-only") == true)
...
/// `opennotes://activate?key=…` pre-fills a key; every other link is left alone.
@Suite("Activate deep link")
struct ActivateLinkTests {
    @Test func readsTheKey() {
        #expect(ActivateLink.key(from: URL(string: "opennotes://activate?key=ABC-123")!) == "ABC-123")
        #expect(ActivateLink.key(from: URL(string: "OPENNOTES://Activate?key=%20ABC%0A")!) == "ABC", "trimmed, case-insensitive")
    }

    @Test func ignoresEverythingElse() {
        for link in ["opennotes://activate", "opennotes://activate?key=", "opennotes://activate?key=%20", "opennotes://share?code=ABC", "macpaper://activate?key=ABC", "https://openapps.space/opennotes/?key=ABC"] {
