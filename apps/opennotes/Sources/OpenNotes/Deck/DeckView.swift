...
struct DeckView: View {
    ...
    private func tabView(_ placement: Placement, progress: MarkdownLite.ChecklistProgress?) -> some View {
        ...
        let label = note.map { ($0.pinned ? "Pinned note: " : "Note: ") + $0.title } ?? tab.id.rawValue
        return TabCard(note: note, title: note?.title ?? tab.id.rawValue, side: content.side, isOpen: isOpen, lifted: lifted, hovered: isHovered, width: tab.frame.width, height: tab.frame.height, progress: progress, ticked: isTicked)
...
private struct TabCard: View {
    ...
    /// The tab keeps the light face in both appearances, so its ink is the
    /// light face's.
    private let ink = Color(nsColor: NSColor(hex: 0x141414))
    ...
    var body: some View {
        ...
        ZStack {
            ...
            // The colour bar on the edge away from the screen's; the
            // checklist's line on the edge towards it.
            HStack(spacing: 0) {
