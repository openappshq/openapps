...
/// Settings → License. States and copy follow LICENSING.md; the price is
/// never shown here, the website states it.
struct LicenseSection: View {
    let license: LicenseController
    /// Scroll target for "Settings → License" (the pill, All Notes' card, the note's footer).
    ...
    var body: some View {
        Section {
            ...
            if let error = license.storageError {
                ...
                note("OpenNotes can’t read or save its free trial record right now. It keeps retrying; check that its Application Support folder is readable and writable. (\(Self.describe(error)))")
            } else if license.journalError {
                note("OpenNotes couldn’t save its license notes in Preferences. It keeps retrying.")
            }
            ...
            MonoLabel("License").id(SettingsNavigation.Anchor.license)
        } footer: {
        ...
        .onChange(of: navigation.request) {
            // "Enter a key" from the card or the footer lands in the field.
            guard navigation.anchor == .license, navigation.wantsKeyField else { return }
            if !needsKeyField { showsKeyField = true }
    ...
    private var statusText: String {
        ...
        case .grace(let days, let warn):
            warn ? "Connect to the internet within \(days) day\(days == 1 ? "" : "s") to keep using OpenNotes" : "Licensed"
    ...
    /// Opens the website, the single place for the price and any offer.
    private var buyButton: some View {
        Button(LicensingConfig.buyURL == nil ? "Buy a license — coming soon" : "Buy a license") {
            if let url = LicensingConfig.buyURL { NSWorkspace.shared.open(url) }
        }
        .buttonStyle(PrimaryButtonStyle())
    ...
    private func confirmRemove() {
        ...
        alert.informativeText = "The activation is freed for another Mac. OpenNotes goes back to its free trial here, and your notes become read-only if the trial has already ended, until you activate again."
