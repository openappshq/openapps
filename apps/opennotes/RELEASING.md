...
The official build is a universal (Apple silicon + Intel) `OpenNotes.app` with
...
"Updates" below).
[`.github/workflows/opennotes.yml`](../../.github/workflows/opennotes.yml) builds it
...
| `scripts/release/release-tag-ruleset.sh check\|apply <definition> [repo]` | Checks for, or creates, the ruleset that makes `opennotes-v*` tags immutable |
| `packaging/homebrew/bump-cask.sh <cask.rb> <version> <sha256>` | Sets the cask's version and digest; the template is `packaging/homebrew/Casks/opennotes.rb` |
...
| Update test | `OPENAPPS_OFFICIAL=1 OPENNOTES_UPDATE_TEST=1`, never with licensing | Compiled out | On, with test hooks; bundle id `com.openappshq.opennotes.updatetest`, its own Application Support folder (`OpenApps/opennotes-updatetest`, where it also keeps its notes — never `~/Documents/OpenNotes`), no URL scheme, no login item, no setup guide |
...
and `bundle.sh` both refuse `OPENNOTES_UPDATE_TEST=1` with
...
`https://openapps.space/opennotes/`, the website page that states the price;
the thanks page deep-links the key back as `opennotes://activate?key=…`, which
only pre-fills the key) and `OPENAPPS_SUPPORT_URL` ("Contact support" on a
revoked license; default `https://openapps.space/opennotes/#questions`, set it
...
compiles the test flavour with `pdt_placeholder_opennotes`, which
`verify-release.sh --release` refuses.

The tests run in every flavour, none of them opening a window, reading the
real record store, touching `~/Documents/OpenNotes` or reaching the network
(every defaults suite is a temporary one, `Tests/OpenNotesTests/TemporaryDefaults.swift`):
...
store under `~/Library/Application Support/OpenApps/opennotes/records/`,
never the Keychain. The core feature is writing notes, and the restriction
is **read-only** (design/products/opennotes.md, "Licensing"): while the
license restricts it, the deck stays visible and every note stays readable,
searchable and exportable, and creating a note, editing text, renaming,
pin / color / face, archiving and unarchiving, reordering, auto-archive and
folder changes wait for a license. Every action asks the controller's
projection of the latest snapshot to the current clocks at the click and
again at every continuation (a close, the folder panel returning, the
auto-archive sweep, the undo toast), and the store asks once more before
its write transaction, so a deadline no timer has delivered yet still
refuses. The one exception: text typed while it was allowed is never lost —
the store stamps that buffer and its flush (the save debounce, a close,
sleep, quit) is written whatever the license says then. An official build
starts restricted until the license controller has bound and projected its
first snapshot, so nothing the launch path runs can write before the
license is known. Settings → License shows the state, Buy a license (the
website states the price, the app never does), the key field and Remove
this Mac; the trial pill sits on the open note, above All Notes, in the
Settings title bar and on the guide's welcome step; All Notes shows the
license card while read-only, the open note's footer and the status menu
the one-line notice.

The setup guide opens once, on the first launch of the packaged app
(welcome with the trial line from the real state, "Nothing to grant", "Your
notes are files", "Starts with your Mac" from the real login setting, tips),
resumes where it was left, and is reachable again from Settings → About →
Show setup guide. Update-test builds and `swift run` builds never open it.
...
decided under their own flag (`FreshInstallDefault`, `OpenNotesCore/FirstRun.swift`):
...
Done for OpenNotes on 2026-09-16 (the pipeline ticket): the certificate is
the shared one and `release/designated-requirement.txt` pins it under
`com.openappshq.opennotes`; the update key's public half is committed in
`release/sparkle-public-key.txt` and its private half is
`SPARKLE_ED_PRIVATE_KEY` on the `opennotes-release` environment; the
environment holds the two publishing credentials and the three variables
(the live product `pdt_0NnjxO8IOA0KbvG88JH6l`, the buy and support URLs);
the tag ruleset "OpenNotes release tags" (#23556146) is active with no bypass
actors; the Dodo brand and product exist in test (`pdt_0NnjxPRw1V6N34ObK6jzN`)
and live. The tap gets `Casks/opennotes.rb` on the first release. What
follows records how, for the next app or a rotation.
...
1. **The certificate**, shared by every OpenApps HQ app (create it once, or
   reuse the existing one), then OpenNotes' pinned requirement derived from it
   ...
   scripts/release/designated-requirement.sh com.openappshq.opennotes \
       ~/openapps-release-signing/release-signing.cert.pem \
       > apps/opennotes/release/designated-requirement.txt
   ```

   Commit `release/designated-requirement.txt`
   (`identifier "com.openappshq.opennotes" and certificate leaf = H"<sha1>"`).
   Every release is signed with exactly this requirement and verified
   against it. OpenNotes asks macOS for no permissions, so a changed certificate
...
2. **The update key**, one per app (done 2026-09-16: the public half is
   committed as `b57a7ea` and the private half is the `SPARKLE_ED_PRIVATE_KEY`
   secret of `opennotes-release`; the temporary folder it was generated in
   was deleted):

   ```sh
   apps/opennotes/scripts/create-update-key.sh ~/opennotes-update-key
   ```

   It writes the private key to `~/opennotes-update-key/sparkle-ed25519.key` and
   the public key to `release/sparkle-public-key.txt` (the second argument, if
   given, is relative to `apps/opennotes`); commit the latter. Official builds
...
active repository ruleset makes `opennotes-v*` tags immutable: no updates, no
...
```sh
scripts/release/release-tag-ruleset.sh apply .github/rulesets/opennotes-release-tags.json openappshq/openapps   # gh must be logged in as an admin
scripts/release/release-tag-ruleset.sh check .github/rulesets/opennotes-release-tags.json openappshq/openapps   # what the workflow runs
```
...
**The tap.** `openappshq/homebrew-tap` holds `Casks/opennotes.rb`. The release
...
**Environment** (created 2026-09-16). The environment **`opennotes-release`**
(Settings → Environments) holds the following. Restrict its
deployment branches and tags to `main` and `opennotes-v*` so nothing else can
...
| Variable | Value |
| --- | --- |
| `OPENAPPS_DODO_PAID_PRODUCT_ID` | OpenNotes' **live** Dodo product (`pdt_0NnjxO8IOA0KbvG88JH6l`) (`pdt_…`, LICENSING.md "Dodo Payments setup"). The release job fails before building when it is unset or not a product ID; `generate-licensing-config.sh` refuses a placeholder for a live build |
| `OPENAPPS_BUY_URL` | Optional (set). Where "Buy a license" opens; the app's default is `https://openapps.space/opennotes/` |
| `OPENAPPS_SUPPORT_URL` | Optional (set). "Contact support" on a revoked license; the app's default is `https://openapps.space/opennotes/#questions`, an empty value hides the link |
...
1. Merge everything the release needs into `main` and make sure the OpenNotes
...
4. The workflow runs four jobs, following RELEASES.md step by step:
   ...
   - `publish` (the only job that can write releases): downloads that exact
     ...
     confirms `opennotes-v0.1.0` names exactly the commit that was built,
     creates a *draft* release, uploads `OpenNotes-0.1.0.zip` and its `.sha256`,
     confirms the tag once more, and only then publishes.
   - `feed`: re-verifies the signed feed against the published zip's digest,
     commits it to `main` as `apps/website/public/updates/opennotes/appcast.xml`
     (the website deploy then serves it at
     `https://openapps.space/updates/opennotes/appcast.xml`), downloads the
     public zip and polls the live feed until both check out, then bumps
     `Casks/opennotes.rb` in `openappshq/homebrew-tap`. The feed and the cask
...
5. Open the release, check the notes, and confirm the upgrade on a Mac with
   ...
   must offer it (and `brew update && brew upgrade --cask opennotes` must work
   too).
...
A manual run (Actions → OpenNotes → Run workflow) from `main` with a version and
...
`https://openapps.space/updates/opennotes/appcast.xml` (`SUFeedURL`) and the
...
fresh install looks for updates and says when one is out — in the status
menu and under All Notes ("OpenNotes X.Y.Z available — Install", "Update
ready — Restart") and in Settings → Updates — but
...
`OpenNotesCore/FirstRun.swift`, the rule "Open at login" already follows); an
...
and `brew upgrade --cask opennotes` keeps working.
...
folder next to the app (`/Applications/.OpenNotes.app.update`, mode 0700); the
...
at once from "Update ready — Restart" (the status menu, All Notes) /
Settings' "Restart to Update". Turning either toggle off cancels a download in flight and discards
...
shows "Move OpenNotes to Applications to enable updates" instead. Updates never
...
```sh
scripts/release/create-signing-certificate.sh /tmp/opennotes-rehearsal          # from the repository root
apps/opennotes/scripts/create-update-key.sh /tmp/opennotes-rehearsal-key release/sparkle-public-key.txt   # the second path is relative to apps/opennotes
scripts/release/designated-requirement.sh com.openappshq.opennotes /tmp/opennotes-rehearsal/release-signing.cert.pem \
    > apps/opennotes/release/designated-requirement.txt
cd apps/opennotes
UNIVERSAL=1 VERSION=0.1.0 RELEASE_SIGNING_P12_FILE=/tmp/opennotes-rehearsal/release-signing.p12 \
  RELEASE_SIGNING_P12_PASSWORD="$(cat /tmp/opennotes-rehearsal/release-signing.p12.password)" \
...
scripts/make-zip.sh
scripts/verify-release.sh --release dist/OpenNotes-0.1.0.zip
SPARKLE_ED_KEY_FILE=/tmp/opennotes-rehearsal-key/sparkle-ed25519.key scripts/make-appcast.sh dist/OpenNotes-0.1.0.zip
...
scripts/bundle.sh
scripts/make-zip.sh
scripts/verify-release.sh dist/OpenNotes-0.1.0.zip    # the requirement is reported, not compared
```
...
`com.openappshq.opennotes.updatetest`, its own Application Support folder
where it also keeps its notes, no URL scheme, no login item, no setup guide
and no licensing — the script
sets `OPENAPPS_LICENSING=0` whatever the shell inherited, and `bundle.sh`
and `Package.swift` refuse `OPENNOTES_UPDATE_TEST=1` with
`OPENAPPS_LICENSING=1`, so it can never reach the record store, the registry
or Dodo, and leaves nothing behind on any Mac — `~/Documents/OpenNotes` is
never touched), both signed with that
...
(a debug build shortens it with `OPENNOTES_DEBUG_TRIAL_DAY_SECONDS=60`), a test
...
`apps/website/public/updates/opennotes/appcast.xml` on `main` from git history
