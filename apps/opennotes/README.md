...
The edges of your notes peek out of the right edge of the screen (or the left), one paper edge per note. Push the pointer against them and the deck fans out: the same tabs, widened in place, one per note. Click a tab and the note slides out to write in; press Escape and it slides back; drag a tab up or down the deck to reorder your notes. The first launch into an empty folder leaves one welcome note, a tour of the formatting and the gestures, yours to edit or archive. The deck stays above every window, full-screen apps and Stage Manager stages included, on every Space.
...
| Deck | `OpenNotes/Deck/` | One non-activating `NSPanel` per hosted display; the state machine drives it; `DeckView` draws the tabs (their edges at rest, the fan when out), the note and the toast from `DeckLayout` |
