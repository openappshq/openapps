...
- **Files, not a database** — each note is one `.md` file in `~/Documents/OpenNotes` (Settings: any folder — iCloud Drive, an Obsidian vault). A short front matter block keeps the color, the face, pinned, archived and the order. Edit a file in another app and the deck picks it up within a second; an outside edit that lands while you have unsaved changes is kept beside the note as a conflict copy, never lost. OpenNotes never deletes a file.
