...
```sh
.build/debug/OpenNotes --preview /tmp/opennotes-preview   # deck states (read-only too), a note with `=` answers, a note with links and the chip, the refusal card, All Notes, the license card, the setup guide, Settings; light and dark
```
...
| Editor | `OpenNotes/Editor/` | `NoteTextView` (plain paste, no substitutions, checkbox clicks, answers drawn after `=` lines and committed by Tab, ⌘-click links and the hover chip) and `NoteStyler` (attributes only, never characters) |
