...
```sh
.build/debug/OpenNotes --preview /tmp/opennotes-preview   # deck states (read-only too, the welcome note, a drag), All Notes (empty, one, ten notes, no match, read-only), the license card, the setup guide, Settings, the paper sheet, a custom colour, three fonts and the two menus; light and dark
```
