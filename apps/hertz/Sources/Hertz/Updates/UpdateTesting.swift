...
/// (`HERTZ_UPDATE_TEST=1`, which also gives them their own bundle
...
///
/// An update-test build never registers a login item and never opens the
/// setup guide: it is a throwaway copy that runs on any Mac and must leave
/// nothing behind but what the test removes.
nonisolated enum UpdateTesting {
    #if HERTZ_UPDATE_TESTING
    ...
    private static let action = ProcessInfo.processInfo.environment["HERTZ_UPDATE_TEST_ACTION"]
    ...
    private static func report(_ line: String) {
        FileHandle.standardError.write(Data("hertz-update-test: \(line)\n".utf8))
    }
    #else
    @MainActor
    static func updateIsReady(installNow: @escaping @MainActor () -> Void, version: String) {}
