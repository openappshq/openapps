...
enum AppResources {
    ...
    static func menuBarImage() -> NSImage {
        ...
        if image.representations.isEmpty {
            return NSImage(systemSymbolName: "waveform.path.ecg", accessibilityDescription: "Hertz") ?? image
        }
        image.isTemplate = true
        image.accessibilityDescription = "Hertz"
        return image
    }
...
/// OpenApps HQ Tactile Studio tokens as used by Hertz (see design/tokens.json).
enum Brand {
    // MARK: Color

    static let accentSolid = dynamic(light: 0x236B48, dark: 0x91DCB4)
    static let accentOn = dynamic(light: 0xFFFFFF, dark: 0x141414)
    static let accentSubtle = dynamic(light: 0xEDF8F1, dark: 0x163A29)
    static let accentText = dynamic(light: 0x236B48, dark: 0xB1E7CA)
    ...
    /// The healthy state shares the accent: Hertz's signature color is the
    /// color of a machine that is fine.
    static let successSolid = accentSolid
    ...
    /// Bricolage Grotesque ExtraBold, for headings and the health score.
    ...
    /// IBM Plex Mono, for readouts and metadata. Digits are tabular, so a
    /// value that changes every two seconds never shifts its neighbours.
