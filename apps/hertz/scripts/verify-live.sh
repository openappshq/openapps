...
# https://openapps.space/updates/hertz/appcast.xml to announce the
...
ZIP_URL="https://github.com/openappshq/openapps/releases/download/hertz-v${VERSION}/Hertz-${VERSION}.zip"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/hertz-live.XXXXXX")"
...
curl -fsSL --retry 5 --retry-delay 10 -o "$WORK/Hertz-${VERSION}.zip" "$ZIP_URL"
actual="$(sha256_of "$WORK/Hertz-${VERSION}.zip")"
...
read -r version _ url sha256 < <(scripts/verify-appcast.sh "$WORK/appcast.xml" "$WORK/Hertz-${VERSION}.zip")
