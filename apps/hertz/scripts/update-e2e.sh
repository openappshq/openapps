...
#      the update-test variant (bundle id com.openappshq.hertz.updatetest,
...
# (the app is a real menu bar app while it runs; it reads the Mac's metrics
# as Hertz always does, and asks for no permission).
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [[ "${1:-}" == "--build-signed" ]]; then
    ...
    build() { # <version> <destination>
        ...
        rm -rf "$2"
        mkdir -p "$(dirname "$2")"
        ditto build/Hertz.app "$2"
    }
    build 1.0.0 "$TMP/Applications/Hertz.app"
    build 1.0.1 "$TMP/B/Hertz.app"
    exit 0
fi
...
BUNDLE_ID="com.openappshq.hertz.updatetest"
SCRATCH_PATH="${SCRATCH_PATH:-.build/update-test}"
export SCRATCH_PATH OPENSSL

if pgrep -f "hertz-update-e2e\..*/Hertz\.app/Contents/MacOS/Hertz" >/dev/null; then
    echo "error: an update-test app from an earlier run is still running; quit it first (pkill -f hertz-update-e2e)" >&2
    exit 1
fi
TMP="$(mktemp -d "${TMPDIR:-/tmp}/hertz-update-e2e.XXXXXX")"
...
for app in "$TMP/Applications/Hertz.app" "$TMP/B/Hertz.app"; do
...
[[ "$(plutil -extract CFBundleVersion raw -o - "$TMP/Applications/Hertz.app/Contents/Info.plist")" == 1000000 ]]
[[ "$(plutil -extract CFBundleVersion raw -o - "$TMP/B/Hertz.app/Contents/Info.plist")" == 1000001 ]]
# A wrongly signed app must not satisfy the requirement (the pinned check has teeth).
cp -R "$TMP/B/Hertz.app" "$TMP/wrong.app"
...
ditto "$TMP/Applications/Hertz.app" "$TMP/A/Hertz.app"

echo "==> 3. Zip, update-sign and announce B"
(cd "$TMP/B" && ditto -c -k --sequesterRsrc --keepParent Hertz.app "$TMP/serve/Hertz-1.0.1.zip")
SPARKLE_ED_KEY_FILE="$TMP/update-key/sparkle-ed25519.key" UPDATE_PUBLIC_ED_KEY="$PUBLIC_KEY" \
    DOWNLOAD_URL="http://127.0.0.1:${PORT}/Hertz-1.0.1.zip" OUT="$TMP/serve/appcast.xml" \
    scripts/make-appcast.sh "$TMP/serve/Hertz-1.0.1.zip"
...
APP="$TMP/Applications/Hertz.app"
BIN="$APP/Contents/MacOS/Hertz"
...
run_app() { # <log> <action>: runs A until it quits by itself (or 2 minutes pass)
    ...
    HERTZ_UPDATE_TEST_ACTION="$2" "$BIN" > "$1" 2>&1 &
    APP_PID=$!
...
still_1_0_0() { # <label>
    ...
    if [[ -e "$TMP/Applications/.Hertz.app.update" ]]; then
...
grep -q 'hertz-update-test: available 1.0.1' "$TMP/run-fresh.log" || { echo "error: the fresh install did not find 1.0.1:" >&2; cat "$TMP/run-fresh.log" >&2; exit 1; }
grep -q 'hertz-update-test: cycle-finished ok' "$TMP/run-fresh.log" || { echo "error: the launch check did not finish cleanly:" >&2; cat "$TMP/run-fresh.log" >&2; exit 1; }
if grep -q 'hertz-update-test: downloading\|hertz-update-test: staged' "$TMP/run-fresh.log"; then
...
if requests_since_probes | grep -q '"GET /Hertz-1.0.1.zip'; then
...
if grep -q 'hertz-update-test: cycle' "$TMP/run-upgrade.log"; then
...
grep -q 'hertz-update-test: revoked-during-download' "$TMP/run-revoke-download.log" \
    || { echo "error: the download was never revoked:" >&2; cat "$TMP/run-revoke-download.log" >&2; exit 1; }
grep -q 'hertz-update-test: staged' "$TMP/run-revoke-download.log" && { echo "error: the update was staged after the revocation" >&2; exit 1; }
...
grep -q 'hertz-update-test: staged' "$TMP/run-revoke-staged.log" || { echo "error: the update was never staged:" >&2; cat "$TMP/run-revoke-staged.log" >&2; exit 1; }
grep -q 'hertz-update-test: revoked-after-staged' "$TMP/run-revoke-staged.log" || { echo "error: the staged update was never revoked" >&2; exit 1; }
...
grep -q 'hertz-update-test: ready 1.0.1' "$TMP/run-update.log" || { echo "error: A never reported the update ready:" >&2; cat "$TMP/run-update.log" >&2; exit 1; }
if ! requests_since_probes | grep -q '"GET /appcast.xml' || ! requests_since_probes | grep -q '"GET /Hertz-1.0.1.zip'; then
...
test ! -e "$TMP/Applications/.Hertz.app.update"
test ! -e "$TMP/Applications/.Hertz.app.previous"
codesign --verify --deep --strict "$APP"
...
rm -rf "$APP"
ditto "$TMP/A/Hertz.app" "$APP"
[[ "$(plutil -extract CFBundleShortVersionString raw -o - "$APP/Contents/Info.plist")" == "1.0.0" ]]
run_app "$TMP/run-restart.log" restart
grep -q 'hertz-update-test: ready 1.0.1' "$TMP/run-restart.log" || { echo "error: no update staged before the restart:" >&2; cat "$TMP/run-restart.log" >&2; exit 1; }
