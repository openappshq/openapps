#!/usr/bin/env bash
# Publishes a verified zip as the GitHub Release for hertz-v<version>,
...
#
#   GH_REPO=owner/repo TAG=hertz-v1.2.3 VERSION=1.2.3 \
...
#   2. an active ruleset makes hertz-v* tags immutable (moves and
...
ZIP="Hertz-${VERSION}.zip"
SUM="${ZIP}.sha256"
[[ "$TAG" == "hertz-v${VERSION}" ]] || { echo "error: TAG ${TAG} does not match VERSION ${VERSION}" >&2; exit 1; }
...
../../scripts/release/release-tag-ruleset.sh check .github/rulesets/hertz-release-tags.json "$GH_REPO"
...
newest="$(printf '%s' "$releases" \
    | jq -r '.[] | select(.draft | not) | .tag_name | select(startswith("hertz-v")) | ltrimstr("hertz-v")' \
...
if ! gh release create "$TAG" --repo "$GH_REPO" --draft --verify-tag --title "Hertz ${VERSION}" --generate-notes >/dev/null; then
