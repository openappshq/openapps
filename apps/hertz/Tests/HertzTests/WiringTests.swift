...
/// manager with Hertz's values (`Licensing.appID`, its paid product) and
/// fake stores and clients, then mapped to what Hertz shows. The package
...
@LicenseActor
struct WiringTests {
    /// Hertz's Dodo product in test mode (the Dodo products artifact).
    nonisolated static let paid = "pdt_0NniFBM9HoAzUZgUkMhz2"
    static let other = "pdt_openreaction_PAID"
    ...
    func activation(_ product: String, name: String = "Hertz", instance: String = "inst_1") -> Activation {
    ...
    /// What the dashboard and the menu bar do with the manager's state.
    ...
    @Test("1. Hertz's paid key activates and is stored, from Trial or TrialEnded", arguments: [false, true])
    func case1_paidKeyActivates(duringTrial: Bool) async {
        ...
        let message = await manager.activate(key: " HERTZ-KEY\n")
        #expect(message == .activated)
        #expect(message.text(appName: Licensing.appName) == "Hertz is licensed on this Mac.")
        #expect(manager.state == .licensed)
        #expect(restriction(manager) == nil)
        #expect(LicenseRestriction.menuBarShowsReadout(for: manager.state))
        #expect(LicenseBadge.label(for: manager.state, appName: Licensing.appName) == nil)
        #expect(store.record?.productID == Self.paid)
        #expect(store.record?.licenseKey == "HERTZ-KEY")
        #expect(client.calls == [.activate(key: "HERTZ-KEY", name: "Mac")])
        #expect(trialStore.record == trialBefore)
    }

    @Test("2. Another app's key is refused with Hertz named, deactivated again, nothing saved")
    func case2_foreignKey() async {
        client.activation = .activated(activation(Self.other, name: "OpenReaction", instance: "inst_x"))
        let manager = makeManager()
        let message = await manager.activate(key: "OR-KEY")
        #expect(message == .wrongProduct(productName: "OpenReaction"))
        #expect(message.text(appName: Licensing.appName) == "This key is for OpenReaction, not Hertz.")
        #expect(store.record == nil)
        #expect(manager.state == .trialEnded)
        #expect(restriction(manager)?.actions == [.buy, .enterKey])
        #expect(client.calls == [.activate(key: "OR-KEY", name: "Mac"), .deactivate(instance: "inst_x")])
    }

    @Test("5. An activation timeout saves nothing and keeps the readings off")
    ...
    func case17_unreadableTrialRecord(error: LicenseStoreError) async {
        ...
        // The dashboard names the storage problem and offers a retry, not a new trial.
        let card = restriction(manager)
        #expect(card?.title == "Can’t read or save the free trial record")
        #expect(card?.actions.first == .tryAgain)
        // Readable again: the stored record decides and the readings return.
    ...
    func case18_wipedRegistryEndsTrial() async {
        ...
        let manager = makeManager()
        #expect(manager.state == .trial(daysLeft: 3)) // provisional: the readings run
        #expect(restriction(manager) == nil)
    ...
    @Test("27. The registry sees Hertz's salted hash, never the hardware UUID or another app's hash")
    func case27_deviceHash() async {
        ...
        #expect(sent.first != TrialDevice.hash(app: "openreaction", hardwareID: device.uuid!))
