 ...
 * copies: the prepared public copy first, then the app package, then the
 * temporary `OPENREACTION_SOURCE_DIR` override.
 */
function sourceFile(publicPath: string | null, sourcePath: string): URL | null {
  const candidates = [
    ...
    process.env.OPENREACTION_SOURCE_DIR &&
      new URL(sourcePath, `file://${process.env.OPENREACTION_SOURCE_DIR.replace(/\/?$/, "/")}`),
  ];
...
export function readEmoji(): EmojiEntry[] {
  if (!emojiFile)
    throw new Error("emoji.json not found; run `pnpm assets` or set OPENREACTION_SOURCE_DIR");
