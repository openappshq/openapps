...
/** The Homebrew line, the alternative for people who already have brew. */
export function brewInstallCommand(cask: string): string {
  return `brew install --cask ${cask}`;
}

/**
 * The install script the website serves for an app (RELEASES.md, "Install
 * script"): `apps/website/public/install/<id>`, regenerated by the release
 * pipeline to pin the newest release. It is published exactly when the cask is,
 * so it shares the cask's gate.
 */
export function installScriptUrl(appId: string): string {
  return `${SITE_ORIGIN}/install/${appId}`;
}

/** The script's source, for anyone who wants to read it before running it. */
export function installScriptSourceUrl(appId: string): string {
  return `${GITHUB_URL}/blob/main/apps/website/public/install/${appId}`;
}

/** The exact line a visitor pastes into Terminal: fetch the script and run it. */
export function installCommand(appId: string): string {
  return `curl -fsSL ${installScriptUrl(appId)} | sh`;
}
...
export interface AppLicensing {
  ...
  /** `curl -fsSL https://openapps.space/install/<id> | sh`, the primary install; null unless `available`. */
  installCommand: string | null;
  /** Where the install script's source is read. */
  installScriptSourceUrl: string;
  /** `brew install --cask <cask>`, the alternative; null unless `available`. */
 ...
 * direct installer, the one-line install otherwise. The wording never promises
 * a file the page cannot offer.
 */
export function installLabel(licensing: Pick<AppLicensing, "downloadUrl">): string {
  return licensing.downloadUrl ? "Download for Mac" : "Install for Mac";
}
...
export function licensingFor(appId: string, options: LicensingOptions = {}): AppLicensing {
  ...
  return {
    ...
    installCommand: available ? installCommand(appId) : null,
    installScriptSourceUrl: installScriptSourceUrl(appId),
