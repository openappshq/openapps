...
struct PrepareCommitTests {
    ...
    func halves() throws {
        ...
        #expect(prepared.images.map(\.displayID) == [1, 2])
        #expect(prepared.images.map(\.format) == [.still, .still])
        ...
        #expect(first == AppliedImage(display: 1, wallpaper: .starter, url: prepared.images[0].url, format: .still))
