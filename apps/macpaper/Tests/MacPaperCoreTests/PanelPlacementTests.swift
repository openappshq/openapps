...
struct PanelPlacementTests {
    ...
    @Test("Without a menu-bar item the column takes the top center, capped by the display")
    func topCenter() {
        let short = PanelGeometry.panelFrame(screenFrame: Self.notchedScreen, visibleFrame: Self.notchedVisible, item: nil, width: 440, contentHeight: 500)
        #expect(short.midX == 756 && short.maxY == 945 - 8 && short.height == 500)

        let capped = PanelGeometry.panelFrame(screenFrame: Self.notchedScreen, visibleFrame: Self.notchedVisible, item: nil, width: 440, contentHeight: 5000)
        ...
        let overDock = PanelGeometry.panelFrame(screenFrame: Self.notchedScreen, visibleFrame: withDock, item: nil, width: 440, contentHeight: 5000)
        #expect(overDock.height == 859, "875 − 16")
        #expect(overDock.minY >= 78, "never off the bottom")
    }

    @Test("The menu-bar item centers the column on itself, clamped to the notched display's right edge")
    func statusItemOnNotchedDisplay() {
        let item = CGRect(x: 1372, y: 952, width: 28, height: 22)
        let frame = PanelGeometry.panelFrame(screenFrame: Self.notchedScreen, visibleFrame: Self.notchedVisible, item: item, width: 440, contentHeight: 400)
        #expect(frame.maxX == CGFloat(1512 - 8))
        #expect(frame.maxY == CGFloat(945 - 8))
    }

    @Test("A display without a notch: the item centers the column under it, an 8-point margin under the menu bar")
    func statusItemOnPlainDisplay() {
        ...
        let frame = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: item, width: 440, contentHeight: 700)
    ...
    func smallDisplay() {
        ...
        let frame = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: item, width: 440, contentHeight: 2000)
    ...
    func secondaryDisplay() {
        ...
        let centered = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: nil, width: 560, contentHeight: 900)
        #expect(centered.midX == 2472 && centered.maxY == 1256 - 8)

        let rightItem = CGRect(x: 3300, y: 1058, width: 28, height: 22)
        let right = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: rightItem, width: 440, contentHeight: 900)
        #expect(right.maxX == CGFloat(3432 - 8))

        let leftItem = CGRect(x: 1512, y: 1058, width: 28, height: 22)
        let left = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: leftItem, width: 440, contentHeight: 900)
    ...
    func widthAndEmptyContent() {
        ...
        let narrowed = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: nil, width: 1000, contentHeight: 300)
        #expect(narrowed.width == CGFloat(600 - 16))
        let empty = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: nil, width: 440, contentHeight: 0)
        #expect(empty.height == 0)
        let negative = PanelGeometry.panelFrame(screenFrame: screen, visibleFrame: visible, item: nil, width: 440, contentHeight: -50)
