...
/// manager with macPaper's values (`Licensing.appID`, its paid product) and
/// fake stores and clients, then mapped to what macPaper shows. The package
...
struct WiringTests {
    ...
    nonisolated static let paid = "pdt_0NnitBSdSZZTDSnKu7j88"
    ...
    func activation(_ product: String, name: String = "macPaper", instance: String = "inst_1") -> Activation {
    ...
    @Test("1. macPaper's paid key activates and is stored, from Trial or TrialEnded", arguments: [false, true])
    func case1_paidKeyActivates(duringTrial: Bool) async {
        ...
        let message = await manager.activate(key: " MACPAPER-KEY\n")
        #expect(message == .activated)
        #expect(message.text(appName: Licensing.appName) == "macPaper is licensed on this Mac.")
        ...
        #expect(store.record?.licenseKey == "MACPAPER-KEY")
        #expect(client.calls == [.activate(key: "MACPAPER-KEY", name: "Mac")])
        #expect(trialStore.record == trialBefore)
    }

    @Test("2. Another app's key is refused with macPaper named, deactivated again, nothing saved")
    func case2_foreignKey() async {
        ...
        #expect(message.text(appName: Licensing.appName) == "This key is for Hertz, not macPaper.")
    ...
    @Test("5. An activation timeout saves nothing and keeps the generator off")
    ...
    func case17_unreadableTrialRecord(error: LicenseStoreError) async {
        ...
        // Readable again: the stored record decides and the generator returns.
    ...
    func case18_wipedRegistryEndsTrial() async {
        ...
        let manager = makeManager()
        #expect(manager.state == .trial(daysLeft: 3)) // provisional: the generator runs
        #expect(restriction(manager) == nil)
    ...
    @Test("27. The registry sees macPaper's salted hash, never the hardware UUID or another app's hash")
