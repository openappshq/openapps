...
| `scripts/update-e2e.sh` | The local end-to-end update test, no secrets, no network beyond 127.0.0.1 (arrives with the updater ticket, modelled on Hertz's) |
...
at once from the popover's "Update ready — Restart" / Settings' "Restart to
...
```sh
scripts/update-e2e.sh   # arrives with the updater ticket
```
...
`com.openappshq.macpaper.updatetest`, no URL scheme, no login item, no setup
guide and no licensing — `bundle.sh` and `Package.swift` refuse
`MACPAPER_UPDATE_TEST=1` with `OPENAPPS_LICENSING=1`, so it can never reach
...
(a debug build shortens a trial day, see `Licensing.swift`), a test
