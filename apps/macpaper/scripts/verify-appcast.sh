...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/macpaper-verify-feed.XXXXXX")"
...
[[ "$(field openapps:app)" == macpaper && "$(field openapps:channel)" == stable ]] || { echo "FAIL: not a macPaper stable feed" >&2; exit 1; }
