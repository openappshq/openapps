...
# https://openapps.space/updates/macpaper/appcast.xml to announce the
...
ZIP_URL="https://github.com/openappshq/openapps/releases/download/macpaper-v${VERSION}/macPaper-${VERSION}.zip"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/macpaper-live.XXXXXX")"
...
curl -fsSL --retry 5 --retry-delay 10 -o "$WORK/macPaper-${VERSION}.zip" "$ZIP_URL"
actual="$(sha256_of "$WORK/macPaper-${VERSION}.zip")"
...
read -r version _ url sha256 < <(scripts/verify-appcast.sh "$WORK/appcast.xml" "$WORK/macPaper-${VERSION}.zip")
