...
#
#   SPARKLE_ED_PRIVATE_KEY=<base64 key> scripts/make-appcast.sh dist/macPaper-1.2.3.zip
#
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/macpaper-appcast.XXXXXX")"
...
PLIST="$WORK/unzipped/macPaper.app/Contents/Info.plist"
test -f "$PLIST" || { echo "error: ${ZIP} does not contain macPaper.app" >&2; exit 1; }
...
DOWNLOAD_URL="${DOWNLOAD_URL:-https://github.com/openappshq/openapps/releases/download/macpaper-v${VERSION}/macPaper-${VERSION}.zip}"
PUBLISHED_AT="${PUBLISHED_AT:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}"
NOTES="${NOTES:-macPaper ${VERSION}. Release notes: https://github.com/openappshq/openapps/releases/tag/macpaper-v${VERSION}}"
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:openapps="https://openapps.space/xml-namespaces/releases">
    <channel>
        <title>macPaper</title>
        <link>https://openapps.space/macpaper/</link>
        <description>macPaper releases</description>
        <language>en</language>
        <item>
            <title>macPaper ${VERSION}</title>
            <openapps:app>macpaper</openapps:app>
...
echo "==> Wrote ${OUT} (macPaper ${VERSION}, build ${BUILD})"
