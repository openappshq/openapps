#!/usr/bin/env bash
# Publishes a verified zip as the GitHub Release for macpaper-v<version>,
...
#
#   GH_REPO=owner/repo TAG=macpaper-v1.2.3 VERSION=1.2.3 \
...
#   2. an active ruleset makes macpaper-v* tags immutable (moves and
...
ZIP="macPaper-${VERSION}.zip"
SUM="${ZIP}.sha256"
[[ "$TAG" == "macpaper-v${VERSION}" ]] || { echo "error: TAG ${TAG} does not match VERSION ${VERSION}" >&2; exit 1; }
...
../../scripts/release/release-tag-ruleset.sh check .github/rulesets/macpaper-release-tags.json "$GH_REPO"
...
newest="$(printf '%s' "$releases" \
    | jq -r '.[] | select(.draft | not) | .tag_name | select(startswith("macpaper-v")) | ltrimstr("macpaper-v")' \
...
DRAFT_ID="$(gh api --method POST "repos/$GH_REPO/releases" \
    -f tag_name="$TAG" -f target_commitish="$BUILT_COMMIT" -f name="macPaper ${VERSION}" \
