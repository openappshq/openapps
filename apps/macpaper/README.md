...
**Wallpapers, made from the menu bar.**
...
A global shortcut (⌥⌘P by default, in the icon's menu and rebindable in Settings) opens the same column under the icon from anywhere, on whatever display the icon is on. The column is as tall as its content, up to what the display allows, and the sections scroll inside it. The setup guide shows where it lives.

macPaper needs **no permissions**: the hotkey is Carbon's `RegisterEventHotKey`, the desktop is set and read through `NSWorkspace`, and the theme comes from a distributed notification. Nothing leaves the Mac. The lock screen follows the desktop (macOS offers no public way to set a separate one); Now Playing art and moving wallpapers are deliberately not built (see the [product contract](../../design/products/macpaper.md)).
...
macOS 14 Sonoma or later, with or without a notch.
...
swift test             # MacPaperCore (generators and pixel fields with golden hashes, palettes, the quality gate, recipes, pins, export, stores, apply, panel geometry, panel rules)
...
To see the UI without changing anything on the Mac, the debug build has a preview harness: it draws the column over four desktops (a saturated mesh, grey, near-black, near-white) at every width, every section, every generator's parameters, the column under the menu-bar item on a notched display and on one without a notch, capped and scrolled on a small display, the guide's panel step, the restricted state and Settings in light and dark to PNGs, with a throwaway defaults suite and a recording desktop applier, and quits:
...
| Panel rules | `MacPaperCore/PanelStateMachine.swift`, `Panel.swift` | The menu-bar item, the shortcut, fullscreen and settings as a pure state machine; the panel's frame under the item inside the display's visible frame as pure geometry |
| App | `MacPaper` | `AppModel` (the draft behind one gated edit entry, previews off the main actor, actions), the status item and its menu, `PanelHost` and `PanelController` (the `NSPanel` under the item), `DesktopKeeper` (the pin), `ThemeWatcher`, `ClockController`, `HotkeyCenter` (Carbon), `ShuffleEngine`, Settings, the login item, the preview harness |
...
| Setup guide, updates | `MacPaper/Onboarding/`, `MacPaper/Updates/`, [`packages/openapps-updater`](../../packages/openapps-updater) | The guide shown once after install (welcome, where the panel lives, nothing to grant, starts with your Mac, tips); the shared updater in official builds, with the fresh-install defaults from `MacPaperCore/FirstRun.swift` |
