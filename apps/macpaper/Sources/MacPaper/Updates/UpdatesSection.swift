...
private struct UpdaterControls: View {
    ...
    var body: some View {
        if updater.location != .updatable {
            note("Move macPaper to Applications to enable updates.")
        } else {
            ...
            if let backup = updater.preservedBackup {
                HStack(alignment: .top) {
                    note("An update could not be completed and the previous version was kept at \(backup.path). If macPaper works, remove it.")
                    Spacer()
    ...
    @ViewBuilder private var statusRow: some View {
        ...
        case .idle, .upToDate:
            HStack {
                note(updater.phase == .upToDate ? "macPaper is up to date." : lastCheckText)
                Spacer()
        ...
        case .available(let item):
            HStack {
                Text("macPaper \(item.version.description) is available.").font(Brand.body(13))
                Spacer()
                Button("Install and Restart") { updater.installAvailable() }
            }
        case .downloading(let item):
            HStack {
                note("Downloading macPaper \(item.version.description)…")
                Spacer()
                ProgressView().controlSize(.small)
            }
        case .staged(let staged):
            HStack {
                Text("macPaper \(staged.item.version.description) is ready and installs when you quit.").font(Brand.body(13))
                Spacer()
