...
/// Accessibility-driven surface choices shared by the panel, the popover
/// and settings.
struct SurfaceStyle {
...
private struct CardSurface: ViewModifier {
    ...
    /// The preview harness draws no glass or material: the opaque surface.
...
private struct SecondaryAction: ViewModifier {
    @Environment(\.accessibilityReduceTransparency) private var reduceTransparency
    @Environment(\.colorSchemeContrast) private var contrast
    /// The preview harness draws no glass: the outlined button stands in.
    @Environment(\.previewRendering) private var previewRendering

    func body(content: Content) -> some View {
        let style = SurfaceStyle(reduceTransparency: reduceTransparency, increasedContrast: contrast == .increased)
        if #available(macOS 26, *), style.usesGlass, !style.increasedContrast, !previewRendering {
            content
                .buttonStyle(.glass)
...
/// Small icon-only action in a card's header: quiet until hovered,
