...
private struct HistoryRow: View {
    ...
    var body: some View {
        HStack(spacing: Brand.Space.s12) {
            Button {
                ...
                HStack(spacing: Brand.Space.s12) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text(Favorite.defaultTitle(for: entry.wallpaper))
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("\(Favorite.defaultTitle(for: entry.wallpaper)), applied \(entry.appliedAt.formatted(.relative(presentation: .named)))")
