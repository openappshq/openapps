...
/// then the grid of preset palettes, the matching one ringed.
struct PaletteSection: View {
    ...
    /// Seven per row; drawn as plain rows so the harness sees every cell.
    private var grid: some View {
        let cell = ((width - gap * CGFloat(columns - 1)) / CGFloat(columns)).rounded(.down)
        let rows = stride(from: 0, to: PresetPalettes.all.count, by: columns).map { Array(PresetPalettes.all[$0..<min($0 + columns, PresetPalettes.all.count)]) }
...
private struct PresetCell: View {
    ...
    var body: some View {
        Button(action: action) {
            LinearGradient(colors: preset.colors.map { Color($0) }, startPoint: .topLeading, endPoint: .bottomTrailing)
