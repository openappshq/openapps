...
/// exposes state for the UI and tells the app whether generating and
/// applying may run.
///
...
/// held clock-behind, an unchecked wake), so every consumer — the panel,
/// Apply, Shuffle, the scheduled shuffle, an export — sees a deadline the
...
/// housekeeping. macPaper’s core feature is shown on the main actor, so
...
final class LicenseController {
    ...
    /// Whether generating and applying may run right now (LICENSING.md:
    /// the core feature). Never cached: the model asks at every action,
    /// the views on every body.
    ...
    func start() {
        ...
        pathMonitor.start(queue: DispatchQueue(label: "space.openapps.macpaper.license.network"))
    }
    ...
    /// The pill in the panel header and the settings title bar, while not
    /// simply licensed. The end of the trial is said here and by the panel's
    /// card; nothing opens on its own.
    var badge: LicenseBadge.Label? {
        LicenseBadge.label(
            for: state,
    ...
    /// The panel's card while the feature is off; nil while it runs.
