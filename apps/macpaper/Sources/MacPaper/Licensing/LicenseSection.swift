...
struct LicenseSection: View {
    ...
    /// Scroll target for "Settings → License" (the pill, the panel's card).
    ...
    var body: some View {
        Section {
            ...
            if let error = license.storageError {
                ...
                note("macPaper can’t read or save its free trial record right now. It keeps retrying; check that its Application Support folder is readable and writable. (\(Self.describe(error)))")
            } else if license.journalError {
                note("macPaper couldn’t save its license notes in Preferences. It keeps retrying.")
            }
        ...
        .onChange(of: navigation.request) {
            // "Enter a key" from the panel lands in the field.
    ...
    private var statusText: String {
        ...
        case .grace(let days, let warn):
            warn ? "Connect to the internet within \(days) day\(days == 1 ? "" : "s") to keep using macPaper" : "Licensed"
    ...
    private func confirmRemove() {
        ...
        alert.informativeText = "The activation is freed for another Mac. macPaper goes back to its free trial here, and stops generating if the trial has already ended, until you activate again."
