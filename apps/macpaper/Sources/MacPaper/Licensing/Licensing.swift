...
nonisolated enum Licensing {
    ...
    /// minutes: `MACPAPER_DEBUG_TRIAL_DAY_SECONDS=60` makes a trial "day" one
    ...
    static var trialTiming: TrialTiming {
        #if DEBUG
        if let raw = ProcessInfo.processInfo.environment["MACPAPER_DEBUG_TRIAL_DAY_SECONDS"]
            ?? UserDefaults.standard.string(forKey: "MacPaperDebugTrialDaySeconds"),
...
nonisolated enum LicensingCopy {
    ...
    static var network: String {
        let stays = "Every wallpaper is made on this Mac and stays here: the documents, favorites and imported images live in Application Support, the renders in the folder you export to."
...
/// show this card in place of the generator, and Shuffle, Apply, Export and
/// a new seed are off. Settings, favorites and Quit always work; the
/// wallpaper already applied stays.
nonisolated struct LicenseRestriction: Equatable, Sendable {
    enum Action: Equatable, Sendable {
        /// Opens the website's macPaper page (`LicensingConfig.buyURL`).
        case buy
    ...
    static func card(for state: LicenseState, storageError: Bool = false, trialStorageError: Bool = false) -> LicenseRestriction? {
        ...
        case .trialUnavailable:
            if storageError {
                return LicenseRestriction(
                    ...
                    detail: "macPaper keeps retrying. Check that its Application Support folder is readable and writable; details are under Settings → License.",
            ...
            return LicenseRestriction(
                ...
                detail: "The generator appears as soon as the trial record is saved. No signup.",
                actions: [.enterKey]
            )
        case .trialEnded:
            return LicenseRestriction(
        ...
        case .checkRequired:
            return LicenseRestriction(
                ...
                detail: "The license hasn’t been checked for a week. One successful check turns the generator back on.",
                actions: [.tryAgain, .enterKey]
            )
        case .revoked:
            return LicenseRestriction(
...
final class LicenseStatus {
    ...
    /// The trial's remaining time or the short reason the feature is off;
