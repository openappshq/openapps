...
extension AppDelegate {
    ...
    /// The `macpaper://` scheme (scripts/bundle.sh registers it). One handler
    /// for every host, dispatched in `openDeepLink`: `activate?key=…` from
    /// the website's thanks page only pre-fills the key, and the user
    /// confirms in Settings → License; builds without licensing ignore it.
    /// A share-code host is added beside it there — never as a second
    /// `kAEGetURL` handler, which would replace this one.
    ...
    /// Routes one `macpaper://` link by host; anything but `activate` with a
    /// key is left alone.
    func openDeepLink(_ url: URL) {
        guard let key = ActivateLink.key(from: url) else { return }
        #if OPENAPPS_LICENSING
