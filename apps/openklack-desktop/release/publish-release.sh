#!/usr/bin/env bash
# Publishes a packaged release as the GitHub Release for openklack-v<version>,
# and only when the tag names exactly the commit the files were built from
# and the zip is byte-for-byte the one that passed verification.
#
#   GH_REPO=owner/repo TAG=openklack-v1.2.3 VERSION=1.2.3 \
#   BUILT_COMMIT=<sha> EXPECTED_SHA256=<digest> RULESET_READ_TOKEN=<token> \
#   DIST=dist release/publish-release.sh [--dry-run]
#
# The release carries OpenKlack-<version>.zip (the cask's download), its
# update signature and SHA-256, and OpenKlack-<version>.app.tar.gz with its
# signature (the in-app update). EXPECTED_SHA256 is the digest the release
# job computed from the zip it verified, carried as a job output rather than
# as a file next to it; a checksum file that travelled with the download
# proves nothing and is ignored. RULESET_READ_TOKEN reads the tag ruleset as
# a caller that is shown its bypass actors (scripts/release/release-tag-ruleset.sh).
#
...
#   1. the downloaded zip has exactly the expected digest, and every other
#      release file is present;
#   2. an active ruleset makes openklack-v* tags immutable (moves and
...
#   6. a draft release is created and the files uploaded — from here on,
...
if [[ -z "${RULESET_READ_TOKEN:-}" ]]; then
    echo "error: RULESET_READ_TOKEN is not set; the tag ruleset cannot be checked for hidden bypass actors (RELEASES.md)" >&2
    exit 1
fi
DIST="${DIST:-dist}"
CREATE_TAG="${CREATE_TAG:-false}"
ALLOW_OLDER="${ALLOW_OLDER:-false}"
ZIP="OpenKlack-${VERSION}.zip"
SUM="${ZIP}.sha256"
ARCHIVE="OpenKlack-${VERSION}.app.tar.gz"
ASSETS=("$ZIP" "$ZIP.sig" "$SUM" "$ARCHIVE" "$ARCHIVE.sig")
[[ "$TAG" == "openklack-v${VERSION}" ]] || { echo "error: TAG ${TAG} does not match VERSION ${VERSION}" >&2; exit 1; }
...
for asset in "$ZIP" "$ZIP.sig" "$ARCHIVE" "$ARCHIVE.sig"; do
    test -s "$DIST/$asset" || { echo "error: ${DIST}/${asset} not found" >&2; exit 1; }
done
...
scripts/release/release-tag-ruleset.sh check .github/rulesets/openklack-release-tags.json "$GH_REPO"
...
newest="$(printf '%s' "$releases" \
    | jq -r '.[] | select(.draft | not) | .tag_name | select(startswith("openklack-v")) | ltrimstr("openklack-v")' \
...
stop_if_dry_run "create a draft release for $TAG (discarding any leftover draft), upload ${ASSETS[*]}, and publish"
delete_drafts
...
if ! gh release create "$TAG" --repo "$GH_REPO" --draft --verify-tag --title "OpenKlack ${VERSION}" --generate-notes >/dev/null; then
...
gh release upload "$TAG" --repo "$GH_REPO" "${uploads[@]}"
